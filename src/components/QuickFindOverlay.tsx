...
import type { SearchCursorStepBackendResult } from './search-panel/types';
import { dispatchNavigateToLine, dispatchSearchClose } from './search-panel/utils';
...
export function QuickFindOverlay({ tab }: { tab: FileTab | null }) {
  ...
  const translate = useCallback((key: Parameters<typeof t>[1]) => t(language, key), [language]);
  ...
  const navigateByStep = useCallback(
    async (step: 1 | -1, options?: { resetAnchor?: boolean }) => {
      ...
      try {
        ...
        const matchLength = Math.max(0, targetMatch.endChar - targetMatch.startChar);
        cursorAnchorRef.current = {
        ...
        setFeedbackMessage(`${translate('quickFind.failedPrefix')} ${errorMessage}`);
      } finally {
        if (requestSerialRef.current === requestSerial) {
          setIsSearching(false);
        }
    ...
    [keyword, restoreInputFocus, tab, translate]
  );
  ...
  return (
    <div
      className="absolute right-2 top-2 z-[80] flex w-[320px] flex-col gap-1 rounded-md border border-border bg-background/95 p-2 shadow-lg backdrop-blur-sm"
    ...
    >
      <div className="flex items-center gap-1">
        <input
          ref={inputRef}
          type="text"
          className="h-8 min-w-0 flex-1 rounded-md border border-input bg-background px-2 text-xs outline-none ring-offset-background placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
