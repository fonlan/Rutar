...
describe("SettingsModal", () => {
  ...
  it("toggles minimap autohide switch from appearance settings", async () => {
    useStore.getState().toggleSettings(true);
    render(<SettingsModal />);
    fireEvent.click(screen.getByRole("button", { name: /Appearance/i }));
    fireEvent.click(await screen.findByRole("button", { name: "Auto-hide Minimap" }));
    await waitFor(() => {
      expect(useStore.getState().settings.minimapAutohide).toBe(false);
    });
  });
