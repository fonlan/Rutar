...
const zhCN: Messages = {
  ...
  'settings.windowsFileAssociationsDesc': '将 Rutar 设为所选后缀的默认编辑器，支持双击直接打开。',
...
const enUS: Messages = {
  ...
  'settings.windowsFileAssociationsDesc': 'Set Rutar as the default editor for selected extensions. Supports double-click open.',
...
export function getSearchPanelMessages(language: AppLanguage) {
  if (language === 'en-US') {
    return {
      ...
      findHistory: 'Recent find terms',
      replaceHistory: 'Recent replace terms',
      historyEmptyValue: '(empty text)',
      searchTargetPlaceholder: 'Target path (file, folder, or wildcard)',
      searchTargetLabel: 'Search target',
      searchTargetPickTitle: 'Choose target file or folder',
      searchTargetPickFileOrFolder: 'Choose file or folder',
      ...
      crossFileNoMatches: 'No matches found in the target.',
      crossFileResultsSummary: (total: number, files: number, scanned: number, totalFiles: number) =>
        `${total} matches in ${files} files · Scanned ${scanned}/${totalFiles}`,
      crossFileGroupMatchCount: (count: number) => `${count} matches`,
      crossFileCompleted: (total: number, files: number) =>
        `Done · ${total} matches across ${files} files`,
      crossFileFileErrorsTitle: (count: number) => `Skipped ${count} files`,
      crossFileEnterToSearch: 'Enter a keyword to search across the target.',
      crossFileReplaceTitle: 'Replace across target',
      crossFileReplacePreviewLoading: 'Counting matches…',
      crossFileReplacePreviewEmpty: 'No matches to replace.',
      crossFileReplaceSummary: (total: number, files: number, totalFiles: number) =>
        `${total} matches in ${files} of ${totalFiles} files will be replaced`,
      crossFileReplaceTargetLabel: 'Target',
      crossFileReplaceFindLabel: 'Find',
      crossFileReplaceWithLabel: 'Replace with',
      crossFileReplaceConfirm: 'Replace All',
      crossFileReplaceCancel: 'Cancel',
      crossFileReplaceApplying: 'Replacing…',
      crossFileReplaceFailed: 'Cross-file replace failed',
      crossFileReplaceSuccess: (total: number, files: number) =>
        `Replaced ${total} matches across ${files} files`,
      crossFileReplaceSuccessWithErrors: (total: number, files: number, failed: number) =>
        `Replaced ${total} matches across ${files} files · ${failed} files skipped`,
      filterAddRule: 'Add Rule',
      filterClearRules: 'Clear Rules',
      ...
      parseEscapeSequences: 'Parse Escapes',
      parseEscapeSequencesHint: 'When enabled, \\n \\r \\t \\\\ in both Find and Replace inputs are parsed as control characters',
  ...
  return {
    ...
    findHistory: '最近查找记录',
    replaceHistory: '最近替换记录',
    historyEmptyValue: '（空文本）',
    searchTargetPlaceholder: '查找目标路径（文件、文件夹或通配符）',
    searchTargetLabel: '查找目标',
    searchTargetPickTitle: '选择查找目标文件或文件夹',
    searchTargetPickFileOrFolder: '选择文件或文件夹',
    ...
    crossFileNoMatches: '目标范围内未找到匹配项。',
    crossFileResultsSummary: (total: number, files: number, scanned: number, totalFiles: number) =>
      `${total} 处匹配，分布在 ${files} 个文件 · 已扫描 ${scanned}/${totalFiles}`,
    crossFileGroupMatchCount: (count: number) => `${count} 处`,
    crossFileCompleted: (total: number, files: number) =>
      `已完成 · ${total} 处匹配 / ${files} 个文件`,
    crossFileFileErrorsTitle: (count: number) => `跳过 ${count} 个文件`,
    crossFileEnterToSearch: '输入关键词以在目标范围内搜索。',
    crossFileReplaceTitle: '跨文件替换',
    crossFileReplacePreviewLoading: '正在统计待替换的匹配数…',
    crossFileReplacePreviewEmpty: '没有需要替换的匹配项。',
    crossFileReplaceSummary: (total: number, files: number, totalFiles: number) =>
      `将在 ${totalFiles} 个文件中的 ${files} 个文件里替换 ${total} 处`,
    crossFileReplaceTargetLabel: '目标',
    crossFileReplaceFindLabel: '查找',
    crossFileReplaceWithLabel: '替换为',
    crossFileReplaceConfirm: '执行替换',
    crossFileReplaceCancel: '取消',
    crossFileReplaceApplying: '正在替换…',
    crossFileReplaceFailed: '跨文件替换失败',
    crossFileReplaceSuccess: (total: number, files: number) =>
      `已替换 ${total} 处 / ${files} 个文件`,
    crossFileReplaceSuccessWithErrors: (total: number, files: number, failed: number) =>
      `已替换 ${total} 处 / ${files} 个文件 · 跳过 ${failed} 个文件`,
    filterAddRule: '新增规则',
    filterClearRules: '清空规则',
    ...
    modeWildcard: '通配符',
    caseSensitive: '区分大小写',
    parseEscapeSequences: '解析转义',
    parseEscapeSequencesHint: '开启后，查找和替换输入中的 \\n \\r \\t \\\\ 会按控制字符解析',
    reverseSearch: '反向搜索',
    prevMatch: '上一个匹配',
    previous: '上一个',
    nextMatch: '下一个匹配',
