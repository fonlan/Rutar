...
describe('App component', () => {
  ...
  it('keeps tab when close-current gesture confirmation is cancelled', async () => {
    const dirtyTab = createFileTab({ id: 'tab-gesture-close-current-cancelled', isDirty: true });
    useStore.setState({
      tabs: [dirtyTab],
      activeTabId: dirtyTab.id,
    });

    vi.mocked(confirmTabClose).mockResolvedValueOnce('cancel');
    vi.mocked(invoke).mockImplementation(
      createInvokeHandler({
        load_config: async () => ({
          language: 'en-US',
